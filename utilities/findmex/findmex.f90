...
program findmex
  ...
  integer           :: maxiter,ncons,cons_atm(4,MAXCONS)
  logical  :: ex
  ...
  if(ex)then
    ...
    if(ios.eq.0)then
      ...
      if(any(cons_atm(1:2,1:ncons)<1) .or. any(cons_atm(3,1:ncons)<0) .or. &
         ...
         any(cons_atm(1,1:ncons)==cons_atm(4,1:ncons)).or.any(cons_atm(2,1:ncons)==cons_atm(3,1:ncons)).or.&
         any(cons_atm(2,1:ncons)==cons_atm(4,1:ncons)).or.&
         any(cons_atm(:,1:ncons)>natm)) then
  ...
  do iter=1,maxiter
     ...
     do i=1,nc
             ...
             if(constype(i) .eq. 3)then
                     ...
                     call oop(natoms,cons_atm(:,i),cgeom,coord_val(i),bval,0);
                     do j=1,4
...
!==================================================================================
!>analysegeom2
! This subroutine prints geometry to gflname
!----------------------------------------------------------------------------------
